                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-light table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            @foreach($sub_modules as $sub_module => $sub_module_list)
                            <tr>
                                <td colspan="2">
                                    <table class="table table-bordered  table-sm">
                                        <tr>
                                            <th width="20%">{{ $sub_module }}</th>
                                            @foreach($sub_module_list as $key => $value2)
                                            <th>
                                               <label for="{{ $key }}">
                                                {{ $value2 }}
                                                <div class="form-check me-3 me-lg-5">
                                                    <input wire:model="permission.{{$key}}" value="true" type="checkbox" id="{{ $key }}" name="{{ $key }}" class="form-check-input permissions" />
                                                </div>
                                               </label>
                                            </th>
                                            @endforeach
